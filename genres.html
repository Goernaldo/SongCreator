...
assert(values.has('songCreator.adminControl.v1'),'Migrierter Admin-Speicher fehlt');
assert(values.has('songCreator.adminControl.preMigrationBackup.v1'),'Sicherheitsbackup vor Migration fehlt');
...
assert.throws(()=>core.updateUser('Testnutzer',{role:'Owner'},'Administrator'),/Owner-Rolle/,'Administrator konnte Owner vergeben');
assert.throws(()=>core.updateUser('Testnutzer',{premium:true},'Benutzer'),/assign_premium/,'Berechtigungsprüfung für Premium fehlt');
core.grantPremium('Testnutzer','30d','', 'Owner');
let premiumUser=core.snapshot().users.find(u=>u.username==='Testnutzer');
assert(premiumUser.premium&&premiumUser.premiumSource==='admin'&&premiumUser.premiumEnd,'Zeitlich begrenztes Premium fehlt');
core.removePremium('Testnutzer','Owner');
premiumUser=core.snapshot().users.find(u=>u.username==='Testnutzer');
assert(!premiumUser.premium,'Premium wurde nicht entfernt');
